...
- **Design system first.** Colors, typography, and effects are defined as CSS variables in each page's `:root`. Use them — never introduce new accent colors. Gold (`--gold`) is trim, not paint.
...
2. Test all three pages (`index.html`, `app.html`, `whitepaper.html`) at desktop and 375 px mobile width — no horizontal overflow, no console errors.
